...
function exibirLivrosPaginados($conn, $destaque,$secaoId) {
  ...
  $livrosPorPagina = 6; // Número de livros por página
  ...
  for ($i = $indiceInicial; $i < min($indiceInicial + $livrosPorPagina, $totalLivros); $i++) {
    ...
    <div class="col-sm-4 mt-5">
      ...
      <p class="card-text"><?php echo $livros[$i]["descricao"]?></p>
      <?php
      $media_avaliacoes = $livros[$i]["destaque"];
      mediaEstrelasDeAvaliacao($media_avaliacoes);
      ?>
      <p class="card-text"><?php echo $livros[$i]["descricao"]?></p>
      <a href="detalhes.php?id_liv=<?php echo $livros[$i]['id_liv']?>" class="btn btn-primary red-color-button" target="_blank">Ver</a>
    </div>
    <?php
  }
  echo '</div>';
